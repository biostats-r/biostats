...
However, if you want to plot a mean on top of the data points, the summary has to be added after the points.
But you will see very quickly if the summary is hidden behind the data points.

Let's have a look at the previous plot, where we added a red point for the mean bill length for each penguin species.
If we had added the `stat_summary()` before the `geom_jitter()` the red point is hidden behind the data.
This can be done intentional, but the means are less visible in this way.
