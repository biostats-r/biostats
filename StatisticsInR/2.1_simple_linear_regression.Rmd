...
```{r setup-data, include=FALSE, echo=F}

# import data for the example
DiveData <- read_csv("../StatisticsInR/Files/DiveDepths.csv", col_names=T)
...
the explanatory variable. These predictions can either be for observed values 
...
```{r figure-1, echo=FALSE, eval=TRUE, fig.cap="Panel A shows an illustration of inferring the effect of temperature on plant height. Panel B shows an illustration of the prediction of a plant height under a 2&deg;C increase in temperature. Created by Emily G. Simmonds", fig.height=150}
include_graphics("./Figures/PlantHeightTemp.png")
```
...
```{r make-fig-2, include=TRUE, echo=FALSE, results='hide'}

fig_2 <- ggplot(Spurious, aes(y=UraniumUS, x=MathsPHD))+
  geom_point()+
...
can dive. All of these variables correlate with body size. 
...
```{r figure-3, echo=FALSE, eval=TRUE, fig.cap="Illustration of marine mammals by Emily G. Simmonds", fig.height=150}
include_graphics("./Figures/MarineMammals.png")
```
...
You can find the dataset [here](https://github.com/emilygsimmonds/biostats/blob/Regression/StatisticsInR/Files/DiveDepths.csv) if you want to try the analysis for yourself. 
...
```{r make-figure-4, include=TRUE, echo=TRUE}
DiveData

fig_4 <- ggplot(DiveData, aes(x=BodySizeKG, y=MaxDepth, color=Group))+
  geom_point()+
  theme_minimal()

```

```{r figure-4, include=TRUE, echo=FALSE, fig.cap="Scatterplot of body size against maximum dive depth for marine mammals. Colours indicate taxonomic group"}

fig_4 + theme_biostats
...
one is continuous numeric (MaxDepth), and the last one is integer (BodySizeKG). 
...
We will change this variable to be numeric to ensure R treats it the right way in our analyses.
...
The line component of the linear regression is defined by two parameters: <span style="color:orange">**$\alpha$**</span> (Greek letter alpha) = the intercept, 
defining where the regression line crosses the y-axis and <span style="color:blue">**$\beta$**</span> (Greek letter beta) = the slope (steepness/gradient), 
...
and saves the output as a 'ModelObject'.
...
This takes theoutput of `lm()`, the model object, 
...
fig_5 <- ggplot(YourData, aes(x=Explanatory, y=Response))+
...
```{r fig-5, include=TRUE, echo=FALSE, fig.cap="Illustration of components of a linear regression. Intercept is in orange, slope is in blue, and the residuals (explained later) are in red"}

fig_5 + theme_biostats
...
regression line.** They are called the **systematic** part of the model, 
...
In the case of a simple linear regression, the error is a normally distributed.
...
In other words, it is not capturing all of the variation in data. 
...
These are highlighted red in \@ref(fig:fig-5).
...
the intercept, 
the slope must change as well to keep the line going through the
($\bar{X}$, $\bar{Y}$) point. 
...
```{r shiny-app-reg, echo = FALSE, message = F}
...
In this case our model estimates a negative slope and for every increase of 
1 kg in body size, marine mammals will have a maximum dive depth that is 
0.005 m less deep. 
...
To get an estimation of the residuals, we use the `residuals()` 
function with the model object as an argument. 
To calculate the variance in our model residuals we use the `var()` 
function with the residuals as an argument.

</br>

```{r get-dive-residuals, include = T, echo =T}
...
For this model the $\sigma^{2}$ = 508212. 
This number is abstract and does not mean anything on its own, 
but it could be used to compare models. 
...
```{r make-dive-plot, include = TRUE, echo = TRUE}
fig_6 <- ggplot(DiveData, aes(x=BodySizeKG, y=MaxDepth))+
...
```{r fig-6, include=TRUE, echo=FALSE, fig.cap="Scatter plot of dive depths against body size. The black line is the regression line predicted from the linear model"}

fig_6 + theme_biostats
...
$\beta$). **A 95% prediction interval tells you, 
...
```{r fig-7, include = TRUE, echo = FALSE, fig.cap="Scatter plot of dive depths against body size. Line is the regression line from a linear model. Shaded area is 95% confidence interval"}
...
This seems ok. But when we look at the prediction interval, 
we see that when we include uncertainty, 
...
```{r fig-8, echo = FALSE, fig.cap="Example of a good (left) and a bad (right) residuals vs fitted plot for a simple linear regression"}
...
```{r fig-9, echo = FALSE, fig.cap="Example of a good (left) and a bad (right) normal QQ diagnostic plot"}
...
There are two examples of scale-leverage plots shown below. If the assumptions
are met, you can expect:
...
```{r fig-10, echo = FALSE, fig.cap = "Example of a good (left) and a bad (right) scale-location plot"}
set.seed(2020)
...
There are two examples of Cook's Distance plots shown below. If the assumptions
are met, you can expect:
...
```{r fig-11, echo = FALSE, fig.cap = "Example of a good (left) and a bad (right) Cook's Distance plot"}
...
we can now check our DiveModel 
...
```{r fig-12, echo = TRUE, fig.cap = "Residuals vs fitted plot for dive depths model"}
...
\@ref(fig:fig-12) looks quite unusual. 
...
are included in this subset. This is quite interesting. 
...
```{r fig-13, echo = TRUE, fig.cap="Normal QQ plot for dive depth model"}
...
These points also stood out in \@ref(fig:fig-12). 
We will need to look into them more in \@ref(fig:fig-15). 

</br>

#### Scale-location

```{r fig-14, echo = TRUE, fig.cap="Scale-location plot for dive depth model"}
...
```{r fig-15, echo = TRUE, fig.cap = "Cook's distance plot for the dive depth model"}
...
and can be found in R using `summary(YOURModel)$r.squared`. 
...
is very good and a value of 1 is suspicious.  
...
```{r make-final-fig-div, echo = TRUE}

fig_16 <- ggplot(DiveData, aes(x=BodySizeKG, y=MaxDepth, color=Group))+
  geom_point()+
  facet_wrap(.~Group)+
  geom_smooth(method='lm')+
  theme_minimal()
...
```{r fig-16, include = TRUE, echo = FALSE, fig.cap = "Plot of the relationship between dive depth and body size faceted by taxonomic group"}

fig_16 + theme_biostats
