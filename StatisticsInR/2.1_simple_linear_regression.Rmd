...
your_data <- as_tibble(data.frame(response = seq(1,30,1),
                                 explanatory = rnorm(30,1:30,3)))

# create an example dataset for plotting
set.seed(2020) # make sure it is the same each time you run code
example_data <- as_tibble(data.frame(x = round(rnorm(100,seq(1,100,1),10),2),
...
fig_2 <- ggplot(Spurious, aes(y=uranium_us, x=maths_phd))+
  geom_point()+
...
fig_4 <- ggplot(dive_data, aes(x=body_size_kg, y=max_depth, color=group))+
  geom_point()+
  theme_minimal()
...
predictions <- augment(model_object)$.fitted

fig_5 <- ggplot(your_data, aes(x=explanatory, y=response))+
...
```{r get-dive-residuals, include = TRUE, echo = TRUE}
dive_residuals <- residuals(dive_model)
sigma2 <- var(dive_residuals)
sigma2
```
...
fig_6 <- ggplot(dive_data, aes(x=body_size_kg, y=max_depth))+
...
fig_16 <- ggplot(dive_data, aes(x=body_size_kg, y=max_depth, color=group))+
  geom_point()+
  facet_wrap(.~group)+
  geom_smooth(method='lm')+
  theme_minimal()
