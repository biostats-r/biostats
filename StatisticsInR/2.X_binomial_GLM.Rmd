...
You should also review the basics of the Binomial distribution [here](). ADD LINK.

</br>

## Introduction

SEPARATION SHOULD BE ADDED SOMEWHERE BUT I'M NOT SURE WHERE - ANY IDEAS? AND
HOW MUCH DETAIL?
...
NEED TO ADD LINE TO READ IN DATA IF WE CAN DECIDE SOMEWHERE TO HOST IT. YOU
CAN GET IT FROM THE PAPER BUT NOT SURE IF WE CAN HOST IT ELSEWHERE.
...
natural logarithm of the odds ratio: $$\frac{p}{1-p}$$. 

</br>

The odds ratio
is the probability of a event occurring over the probability it doesn't 
occur. Odds are often used in the things like betting, e.g. to represent the 
...
(see \@ref(fig:response-scale-example)). 

```{r response-scale-example, include = TRUE, echo = FALSE, fig.cap = "Example of a Binomial GLM model line on the scale of the response data. Intercept in orange and slope in blue", fig.height=5}
...
(like above in \@ref(fig:response-scale-example)) or
...
```{r plot_ci_example, fig.caption = "Examples of plots of fitted GLM line and 95% confidence intervals (grey shaded area) on the link scale (A) and on the response scale (B)"}
...
```{r sheep_predictions_plot, fig.caption = "A. plot of the fitted GLM line on the link scale including 95% confidence interval (grey), B. plot of the fitted GLM line on the response scale including 95% confidence interval (grey)"}
...
```{r example_predictions_plot, fig.caption = "plot of the fitted GLM line on the response scale including 95% confidence interval (grey)"}
...
If you look carefully at \@ref(fig:example_predictions_plot) you can see that 
...
```{r example_predictions_plot_proportion, fig.caption = "plot of the fitted GLM line on the response scale with proportional response, including 95% confidence interval (grey)"}
...
```{r sheep_predictions_plot2, fig.caption = "Plot of the fitted GLM line on the response scale including 95% confidence interval (grey)"}
...
```{r sheep_predictions_plot3, fig.caption = "Plot of the fitted GLM line on the response scale including 95% confidence interval (grey)"}
...
```{r sheep_predictions_plot4, fig.caption = "Plot of the fitted GLM line on the response scale including 95% confidence interval (grey)"}
...
ANYONE KNOW HOW TO REMOVE THIS TITLE?
