...
and gives us the maximum likelihood estimates of $\color{orange}\alpha$ and
$\color{blue}{\beta_is}$ as an output. 
...
GLMModelObject <- glm(response ~ explanatory, data = your_data,
                     family = binomial(link = "logit"))

```

Running the `glm()` as illustrated above runs the GLM 
and saves the output as a `GLMModelObject`.
...
Let us take a look at the maximum likelihood estimates of our model parameters ($\color{orange}\alpha$, $\color{blue}{\beta}$) 
using the `coef()`.
...
section above: 
$\color{orange}\alpha =$ the intercept; $\color{blue}{\beta_i} = $ 
...
**$\color{blue}{\beta} \color{blue}{, the slope}$**
...
example_data <- mutate(example_data,
                       logity = predict.glm(GLMModelObject, 
...
ggplot(example_data, aes(x = explanatory, y = logity))+
 labs(x = "x",
  ...
  geom_line(aes(x = explanatory, y = logity),
            colour = "blue")+
  geom_point(aes(x = 0, y = coef(GLMModelObject)[1],
...
coef(GLMModelObject)[1]

# convert to original scale (a probability of success)

1/(1 + exp(-(coef(GLMModelObject)[1])))
...
newdata <- data.frame(explanatory = -5:20)

newdata <- mutate(newdata, 
                  predictions = predict(GLMModelObject, newdata))

newdata$predictions2 <- exp(newdata$predictions)/(1+exp(newdata$predictions))

example_data <- mutate(example_data, 
                       predictions = predict(GLMModelObject,
                                             example_data,
                                             type = "response"))
...
ggplot(example_data, aes(x = explanatory, y = response))+
 ...
 labs(x = "x",
  ...
  geom_line(aes(x = explanatory, y = predictions),
            colour = "blue")+
  geom_point(aes(x = 0, y = 1/(1+exp(-(coef(GLMModelObject)[1]))),
                 colour = "orange"), size = 4)+
  theme(legend.position = "none")
...
odds_y_x2 <- coef(GLMModelObject)[1] + (coef(GLMModelObject)[2]*2)
odds_y_x3 <- coef(GLMModelObject)[1] + (coef(GLMModelObject)[2]*3)
