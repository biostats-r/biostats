...
# load the sheep data
sheep_data <- read_csv("../StatisticsInR/Files/SheepData.csv")
...
response to try and improve linearity or equalize the variance - 
...
GLMs for the major part of most biological analyses as a lot of biological data
are non-normal. 

A Binomial GLM still wants to:
...
```{r make-figure1, include = TRUE, echo = FALSE}

fig_1 <- ggplot(aes(x, y), data = example_data)+
  geom_point(shape = 16)+
  geom_smooth(method="glm", method.args = list(family="binomial"), 
              se=TRUE)

```
```{r figure-1, include = TRUE, echo = FALSE, fig.cap = "Example of a fitted Binomial GLM model. The estimated relationship is plotted on the scale of the data."}
...
The data consists of five variables: Year (year of recording), Age (in years),
Survival (0 = died, 1 = survived), Weight (kg) and 
PopSize (number of females in the given year). 
...
First we should have a look at the data using a plot. 

```{r make-sheep-data-plot, include = TRUE, echo = TRUE}

fig_2 <- ggplot(sheep_data, aes(x = Weight, y = Survival))+
 geom_point()+
 ylab("Survival")+
 xlab("Weight of sheep (kg)")+
 theme_minimal()
 
```

```{r figure-2, include = TRUE, echo = FALSE, fig.cap = "Scatterplot of body weight (kg) and against survival of sheep"}

fig_2

```

This gives us a fair idea of what our data looks like. The $Y$ values are all
either 0 or 1. 

You should note that four of our variables are integers: 
Year, Age, Weight and PopSize and one of them is a factor/binary (Survival).

The weight of the sheep is actually a numeric variable as sheep can have weights 
that are between whole kilos. You can change this in R, 
so that it will be treated as a numeric variable.

```{r sheep-data-set-up, include = TRUE, echo = TRUE}
...
Since we are interested in answering:

**Does body weight (kg) influence the survival of sheep?**

we only need two of the five variables in the data: `Weight` and `Survival`. 
...
The explanatory variable is `Weight`, which is numeric. 
...
### Theory
Every model that is created needs to be represented mathematically. 
This representation describes how it was generated.

As we discussed for the linear models, we make the assumption that there is a 
linear relationship between our two variables. 
Hence, we can capture the data generation process with a straight line and 
some error. 
The GLM can be written as an equation in terms of $Y$:

$$
Y_i = \color{orange}\alpha + \color{blue}{\beta_1} X_{1i} + \color{blue}{\beta_2} X_{2i} + \ldots + \color{blue}{\beta_n} X_{ni} + \color{red}\epsilon_i
$$

where $n$ is the number of explanatory variables in the model. 

#### Components of a GLM

There are three main parts of the GLM:

##### The random part

This is the data part of the model. It is assumed with a distribution, 
and in our case a Binomial distribution. 
Thus we choose the distribution based on the data we have. 
The random part is how we deal with the errors $\color{red}\epsilon_i$. 
In the Binomial GLM, the Binomial distribution is assumed for the random part. 
Let us take some time to have a recap of the Binomial distribution.

###### The Binomial Distribution

The Binomial distribution gives the probability of success from two possible 
outcomes. This probability is beounded between $0$ and $1$. 
Examples of such data includes survival date, sex ratio data, land or sea. 

**Parameters**
The parameters of the Binomial distribution is the probability ($p$). 
...
This is the part of the model for each data point. 
This is also known as the linear predictor. 
An example is 
$\color{orange}\alpha + \color{blue}\beta_1 X_{1i} + \color{blue}\beta_2 X_{2i} + \ldots + \color{blue}\beta_n X_{ni}$.

**The Link function**

The link function transforms the model (linear) onto the scale of data. 
This connects the systematic part of the model to the random data. 
It describes how the mean depends on the linear predictor. 
Different distributions have different link functions.

The default link function or canonical function is the logit function. 
The logit function is defined as $log\bigg(\frac{\mu}{1-\mu}\bigg)$. 
Other common link functions are the **probit** and **cloglog** functions.

```{r make-logit-plot, echo = FALSE}
...
```{r figure-3, include = TRUE, echo = FALSE, fig.cap = "The Logit function"}
 ...
 * The correct variance function is used.
...
sheepModel <- glm(Survival~ Weight, data = sheepdata, family = "binomial")
...
ggplot(exampledata, aes(x= Weight, y = as.numeric(Survival)-1))+
...
ggplot(exampledata, aes(x= Weight, y = as.numeric(Survival)))+
  geom_point(colour = "grey70")+
...
newdata <- data.frame(Weight= sheepdata$Weight)
...
```{r}
ggplot(sheepdata, aes(x=Weight, y=Survival))+
  geom_point(colour = "grey70")+
...
```{r, echo = TRUE}
newdata <- data.frame(Weight=exampledata$Weight)
ExampleIntervals <- predict(ExampleModel, newdata= newdata, type="link", se=TRUE)

Intervals <- data.frame(cbind(Weight = exampledata$Weight,
...
ggplot(data = Intervals, aes(x=Weight, y= fit))+
  ...
  geom_point(data=exampledata, aes(x=Weight, y=Survival))+
...
Intervals <- data.frame(cbind(Weight = sheepdata$Weight,
...
ggplot(data = Intervals, aes(x=Weight, y= fit))+
  geom_line(color="blue")+
  geom_point(data=sheepdata, aes(x=Weight, y=Survival))+
       geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.5)+
  ylab("Survival Probability")+
  theme_minimal()
...
```{r, echo=T, message=F}
sheepdata[c(47,107,392),]
```

```{r}
hist(sheepdata$Weight, main="")
```
