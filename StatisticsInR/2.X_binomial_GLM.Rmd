...
From this plot, we can test a few things: that the relationship between 
X and Y is linear (on the link scale), the residuals have (approximately) 
a mean of 0, and the variance of the residuals is equal for all fitted values 
(homoscedasticity). 
These are not key assumptions of a GLM, even though they
were for a linear model. However, once we have scaled the residuals, these 
characteristics are something we would expect to be approximately true. 

**Note that this will be very approximate! 
The plots could in fact look quite bad.**

Remember that we use the scaled residuals 
(Pearson or Deviance) instead of the original residuals.
The original residuals would not meet any of these assumptions. 


</br>

```{r fitted_vs_residuals_example, eval = FALSE}

# extract fitted values
# and add to a data frame with the residuals
fitted <- tibble(fitted = fitted(glm_model_object),
                 residuals_pearson = residuals_pearson,
                 residuals_deviance = residuals_deviance)

pearson <- ggplot(data = fitted, aes(x = fitted, 
                              y = residuals_pearson))+
geom_point(colour = "grey")+
geom_hline(yintercept = 0)+
labs(title = "Pearson")

deviance <- ggplot(data = fitted, aes(x = fitted, 
...
the log odds of the survival of the sheep increases by a log odds of
...
The confidence interval limits are same signs, 
