...
example_data2 <- example_data |>
  ...
  mutate(
    explanatory = sample(1:100, 10, replace = TRUE),
...
ggplot(aes(x = explanatory, y = response), data = example_data) +
  geom_point(shape = 16) +
  geom_smooth(
    method = stats::glm, method.args = list(family = "binomial"),
    se = TRUE
  )
```
...
glm_model_object <- glm(response ~ explanatory,
  data = your_data,
  family = binomial(link = "logit")
)
...
sheep_model <- glm(Survival ~ Weight,
  data = sheep_data,
  family = binomial(link = "logit")
)
```
...
#| echo: false
#| fig-cap: >
#|   Example of a Binomial GLM model line on the logit scale.
#|   Intercept in orange and slope in blue
...
#| echo: false
#| fig-cap: >
#|   Example of a Binomial GLM model line on the scale of the response data. 
#|   Intercept in orange and slope in blue
#| fig-height: 5
...
ggplot(example_data, aes(x = explanatory, y = response)) +
  ...
  geom_point(aes(
    x = 0, y = 1 / (1 + exp(-(coef(glm_model_object)[1]))),
    colour = "orange"
  ), size = 4) +
  theme(legend.position = "none")
```
...
predictions <- newdata |>
  mutate(
    predictions_link = predict(glm_model_object,
      newdata = newdata,
      type = "link",
      se.fit = TRUE
    )$fit,
...
predictions <- predictions |> mutate(
  upper_ci_link = predictions_link + (2 * SE_link),
...
example_predictions_link <- ggplot(
  data = predictions,
  aes(
    x = explanatory,
    y = predictions_link
  ...
  labs(
    title = "A. Predictions on link scale",
...
example_predictions_response <- ggplot() +
  ...
  labs(
    title = "B. Predictions on response scale",
    y = "Probability",
    x = "Explanatory"
  )
...
sheep_predictions_link <- ggplot(
  data = sheep_predictions,
  aes(
    x = Weight,
    y = predictions_link
  )
...
#| fig-cap: >
#|   The fitted GLM line on the response scale 
#|   including 95% confidence interval (grey)
...
ggplot() +
  ...
  labs(
    title = "Model predictions on response scale",
    y = "Probability of survival",
    x = "Body weight (kg)"
  )
```
