...
A **merge conflict** can occur when you are changing the same line in one file differently, for example in two different **branches**.
...
**Merge conflicts** can easily be avoided if keep your **branches** separate for each file.

But **merge conflicts** cannot always be avoided and you will get notified in the **Terminal**.
First, do not panic.

The terminal message will let you know which branch and file is conflicting.
Get an overview of what is going on and fix the problem in one of the **new branches**.

That should have fixed the problem.
Go ahead, **commit**, switch to the **main branch**, **merge** and **push** to GtiHub.
