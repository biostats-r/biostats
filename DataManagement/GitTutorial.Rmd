...
This also means that all your repositories that are public can be seen by everybody and it is therefore important not to commit sensitive information like passwords.
...
All the files that already exist in your repo will show up with yellwo question marks and are ready to be pushed to github.
...
Commits are cheap.
It does not take much time to click on **Commit**, stage the file and write a few words about the commit (aim for max 50 characters).
...
We strongly suggest to use forks and branches when collaborating on a project (see \@collaborating-with-forks-and-branches Collaborating with forks and branches tutorial for more details).
<br/><br/>
...
If you have forked a repo it will also show you the upstream repo (see \@collaborating-with-forks-and-branches Collaborating with forks and branches tutorial for more details).
...
If you have added a file (*myfile*) and committed and now you want to remove the file from your last commit, then type:
...
**Delete or remove my last commit**

This only works if you have **not pushed** yet, then type:
```
