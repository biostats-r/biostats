...
If there is simply too much code, you can move code into another file.
There are two ways to do this. 
The first is with [includes](https://quarto.org/docs/authoring/includes.html). 
These effectively copy and paste text and code from another quarto file.
To include the file "_load-data.qmd", which has a code block for loading the data, we need this shortcode.
...
The shortcode needs to be on its own line, with space above and below. 
The file name should start with an underscore.
Includes could also be useful for different sections of a manuscript


The second method is to use the file option in a code block. 
For example, to run the code from the file "load-data.R", we could use the following code block.
