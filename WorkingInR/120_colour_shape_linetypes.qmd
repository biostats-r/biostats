...
#| label: wrong-aes
#| message: false
#| fig-cap: Use fill for areas.
#| fig-height: 3
#| fig-width: 3.5
#| fig-alt: >
#|   Histogram of penguin bill lengths. The histogram bars are dark grey bars,
#|   while the lines around the bars are coloured by species. 
ggplot(penguins, aes(x = bill_len, colour = species)) +
  geom_histogram()
```
...
```{r}
#| label: fill-colour-set
#| fig-height: 3
#| fig-width: 3.5
#| fig-alt: >
#|   Histogram of penguin flipper lengths, with pink bars surrounded by purple
#|   lines.
...
#| label: qual-fills
#| fig-alt: >
#|   Density plot of penguin flipper length with the default fills for each 
#|   species.
...
#| fig-height: 3
#| fig-alt: >
#|   Density plot of penguin flipper length with the manually selected fills
#|   for each species.
ggplot(penguins, aes(x = flipper_len, fill = species)) +
  geom_density() +
...
#| label: qual-brewer
#| fig-height: 4
#| fig-alt: The qualitative colour palettes available from RColorBrewer
...
#| fig-height: 3
#| fig-alt: >
#|   Density plot of penguin flipper length with the Set1 palette from 
#|   RColorBrewer.
ggplot(penguins, aes(x = flipper_len, fill = species)) +
  geom_density() +
...
#| label: seq-brewer
#| fig-height: 4
#| fig-alt: The sequential colour palettes available from RColorBrewer.
...
#| label: div-brewer
#| fig-height: 4
#| fig-alt: The diverging colour palettes available from RColorBrewer
...
#| fig-width: 3.5
#| fig-alt: >
#|   Scatterplot of penguin bill length against body mass, with Adelie penguins
#|   highlighted in red.
library(gghighlight)
...
About 1/12 men and 1/200 women have colour vision deficiency.
We need to use colour and fill palettes that are colour-blind friendly.
Some popular scales are not. 
For example, we can use the `colorBlindness` package to simulate how this figure is perceived by people with different forms of colour vision deficiency.

```{r}
#| label: not-colourblind-friendly
#| fig-cap: Default ggplot fill.
#| fig-height: 3
#| fig-width: 3.5
#| fig-alt: > 
#|   Density plot of penguin flipper lengths by species, 
#|   with the default fill colours.
p3 <- ggplot(penguins, aes(x = flipper_len, fill = species)) +
...
```{r}
#| label: colour-blindr-default
#| echo: true
#| fig-cap: How people with different types of colourblindness see the default colour
#|   scale.
#| fig-alt: > 
#|   Simulation of how the previous figure would be seen by people with
#|   different types of colour vision deficiency.
...
```{r}
#| label: viridis-colourblind
#| message: false
#| fig-cap: Viridis fill.
#| fig-height: 3
#| fig-width: 3.5
#| fig-alt: Repeat of the previous figure with a colour-blind friendly palette.
...
```{r}
#| label: viridis-colourblind2
#| echo: false
#| fig-cap: >
#|   How people with different types of colourblindness see the viridis colour
#|   scale.
#| fig-alt: >
#|   Simulation of how people with different types of colour blindness would see
#|   the figure with colour-blind friendly palette.
#|   
cvdPlot(p3_v)
```
...
#| fig-width: 5
#| fig-height: 2
#| fig-cap: Alpha values
#| fig-alt: >
#|   Overlapping circles with different values of alpha setting the
#|   transparency.
...
#| fig-height: 3
#| fig-alt: >
#|   Scatterplot of penguin bill depth against bill length with crosses instead
...
#| echo: false
#| fig-cap: Shapes
#| fig-alt: The different shapes available for plotting and their numeric codes. 
#| fig-height: 2.5
step <- 9
...
```{r}
#| label: scale-shape-manual
#| fig-width: 4.5
#| fig-height: 4
#| fig-alt: >
#|   Scatterplot of penguin bill length against body mass with manually chosen
#|   shapes for each species.
ggplot(
  penguins,
  aes(x = body_mass, y = bill_len, colour = species, shape = species)
) +
  geom_point() +
...
#| fig-width: 3.5
#| fig-alt: >
#|   Plot of chick weight over time for different diets. Each chick is
...
#| fig-alt: > 
#|   Plot of chick weight over time for different diets. Each chick is
#|   represented by a line, with a different line type for each diet.
ggplot(
  ChickWeight,
...
#| fig-alt: Five linetypes and their names.
#| echo: false
