...
The code for a scatterplot looks something like this:
...
The `mapping` uses `aes()` to describe how the variables should be mapped onto the **aes**tetics, such as x and y location or colour.
...
These other elements include the graphical elements that display the data, usually created with a `geom_*()` function, labels created with `labs()`, and [themes](themes) that change the non-data elements of the plot with `themes()`.
...
We will mostly work with the [Palmer Penguin](palmerpenguins) data set.
...
If you want to choose a specific colour, this has to be done in geom or in scales.
...
The legend is automatically plotted on the right side.
...
To come back to the previous point, if you want to make all the points red this can be done in `geom_point()` by setting `colour = "red".
Note that this time we chose `shape = species`, another aesthetics to use different symbols for the three species.

```{r red, eval = TRUE, fig.height = 3, fig.width = 3.5}
