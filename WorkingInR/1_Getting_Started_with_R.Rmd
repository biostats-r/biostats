...
`r fontawesome::fa(name = "r-project", fill = "steelblue")` is powerful statistical and graphical tool which is available for most platforms (Windows, Mac, Linux, etc.). 
...
It is the engine that will do all your analyses, but does not look very user-friendly.

Fortunately, other programs can fix that.
...
## Installing R and RStudio {.panelset}
...
You will learn extensively about R objects and data in the \@ref(first-steps-in-r).
...
You will find this list in the tab `Packages` (see section \@ref(the-packages-tab)).
...
Now load the package with `library(biostats.tutorials)` and the tutorials should appear in the tab `Tutorial` (see section \@ref(the-tutorial-tab)).
...
<details>
<summary>Using R < 4.1?</summary>
Ideally upgrade R to the latest version. 
If you cannot, run this code to modify `biostats.tutorials` so that it can run on your computer.
```{r old-pipes, eval = FALSE}
library(tidyverse)
old_pipes <- function(){
...
In the chapter First Steps in R (\@ref(first-steps-in-r)), you  will learn to write in the R language. 
