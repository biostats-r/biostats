...
+ R allows for reproducible research: anyone running you code on your data will get the exact same result as you do. 
+ R allows for sharing your code and collaborating with others on a project.
+ R code is easy to publish. 
+ R contributes to replicability: anyone can use your published code on another data set to replicate your results (or not).
+ etc.
...
This interface is obviously more complexe and organized, with many more menu items and buttons. 
...
Projects let you organize your tasks and load the files and packages that you define as necessary for the workflow.  
...
```{r rstudio-gui3, echo = FALSE, fig.cap="_The tabs_ Console _and_ Terminal _are located in bottom left pane._", out.width="100%"}
...
The tab `Terminal` allows for manipulating files on your disk, running Python scripts, etc (see Figure \@ref(fig:rstudio-gui5)).
...
The tab `Packages` provides you with a list of all the R packages that are currently installed on your machine (see Figure Figure \@ref(fig:rstudio-gui11).
