...
Below is an example of a non-tabular dataset. Here, each profile contains two lines with a `field: value` pair: 
...
If you use `col_names = FALSE`, the first row of the data file will be the observation in the resulting tibble, and the variables will be named `X1`, `X2`, etc.

```{r col-names0, echo=TRUE} 
weather <- read_delim(file = "data/weather.csv", 
                      delim = ",", 
                      col_names = TRUE)
```

Otherwise, `col_names =` may be followed by the actual variable names that you want to use.
