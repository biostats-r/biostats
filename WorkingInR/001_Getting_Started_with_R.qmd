...
We will further describe RStudio's interface in @sec-the-interface and how to set up projects in @sec-working-with-a-project.
...
### On to your own computer

Use this option if

- You have permission to install software on your computer (i.e. it is not a university-owned clien computer)
- Your computer is fast enough, and has enough hard disk space, to run R and RStudio (if not use the UiB virtual computer or rstudio.cloud)
...
Simply go to [apps.ntnu.no](https://apps.ntnu.no), search for R and RStudio and install both on your computer.
...
If you have a client machine but do not have access to a Software Center, you need to ask your IT-department for help installing R and RStudio (you can use rstudio.cloud until your computer is fixed).
...
[rstudio.cloud](https://rstudio.cloud) is a web-service provided by RStudio which lets you run R and RStudio on their computer through a browser tab.
...
There is a [free plan](https://rstudio.cloud/plans/free), but you will probably need a [paid plan](https://rstudio.cloud/plans/plus) (few dollars a month) if you want to use it throughout your course.
[rstudio.cloud](https://rstudio.cloud) is not suitable for large analyses as it has limited memory.

To set up an account, go to [rstudio.cloud](https://rstudio.cloud)

:::

### Exercise {- .facta .toc-ignore}

Install R and RStudio either on your laptop (UiB students can use [Third Party Portal](https://apps.uib.no) instead).
You need R version 4.1 or newer and a recent version of RStudio.
If you installed R or RStudio more than a few months ago, you will need to reinstall it.
...
In @sec-working-script, we will talk more about working with scripts and adding comments.
...
## Working with a project {#sec-working-project}
...
It is saved automatically when RStudio shuts down, and restored when it starts up again.
We advise you to prevent RStudio from saving changes and restoring .Rdata to improve reproducibility.

Go to Tools > Global Options... > General > Basic to get to the menu shown in @fig-settings1.
...
Or, if you install the `usethis` paackage, you can do this in code with
...
Go to Tools > Global Options... > Code > Saving to get to the menu shown in @fig-settings2.
In the section Serialization (Fig. @fig-settings2, red box), click "Change" and choose UTF-8.

```{r}
#| label: settings2
#| echo: false
#| fig-cap: "Saving files as UTF-8"
#| fig-alt: Screenshot of RStudio Options menu
#| out-width: "100%"
knitr::include_graphics("figures/RStudio-settings2.png")
```
...
#| label: fig-settings3
#| echo: false
#| fig-cap: "Activating soft-wrapping of R scripts"
#| fig-alt: Screenshot of Options menu in RStudio for softwrapping scripts
#| out-width: "100%"
