---
title: "A Bestiary of Brackets and other R notation"
description: What does all the puntuation in R mean
output:
...
knitr::opts_chunk$set(echo = TRUE, rows.print = 3, message = FALSE)
library("tidyverse")
...
Single square brackets `[]` are used for subsetting vectors, matrices and similar objects.

```{r square-bracket}
month.name[6:7] # extract the sixth and seventh element of month.name
...
If `a` and `b` are logical vectors (ie TRUE and FALSE). or vectors that can be coerced to logical vectors, then
...
```
It is probably a good idea to always use brackets in statments with multiple Boolean operators to make the order of evaluation clear to the reader. 
...
The greater than sign is also used in the `tidyverse` pipe `%>%` and the new base R (version 4 and later) pipe `|>`.
...
This can be re-witten as a regular function with ```%/%`(13, 5)``
