...
The `penguins` dataset is used as an example.
...
```{r is.na.sum}
penguins |>
  summarise(n_missing = sum(is.na(bill_len)))
```
...
For example, to remove rows in the `penguins` data set with an `NA` in the `bill_len` or `bill_dep` columns, we could use
...
```{r filter-na}
penguins |>
  filter(species == "Gentoo", !is.na(bill_len))
```
...
penguins |>
  mutate(
    bill_len = replace_na(
      data = bill_len,
      replace = mean(bill_len, na.rm = TRUE)
    )
  )
```
...
mod <- lm(bill_len ~ body_mass, data = penguins)
...
mod <- lm(bill_len ~ body_mass, data = penguins, na.action = na.exclude)
