...
Get a `sf` object from `rnaturalearth::rnaturalearth::ne_coastline()` and find out what coordinate reference system it uses.
...
Make a map with `ggmap` that shows your favourite holiday destination.
...
::: callout-tip
## `terra` vs `raster` packages

The `terra` package is an update to the `raster` package. 
...
A scalebar can be useful, especially for large scale map. On small scale maps, they can be inaccurate as the scale varies.
