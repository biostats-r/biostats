...
Most help files have a similar structure (although some sections are optional).
 ...
 You can run the functions either by copy-pasting them, or using the function `example`.
...
See section \@ref(biostatstutorials) for how to install `biostats.tutorials` and run the tutorials.

### Viewing object content

As introduced in section \@ref(the-environment-tab), the `Environment` tab of RStudio lists all the objects stored in memory in a given project.
...
```{r distr, echo = FALSE, fig.cap = "Different distributions of 1000 random numbers, with default option"}
n <- 1000
tibble(
...
The sequences are not strictly random but pseudo random.
The sequences repeat with a very long period (2^19937^ - 1 for the default Mersenne-Twister algorithm).
...
Be warned that this is bad coding practice, which may result in (or be the origin of) errors that may compromise your work and the validity of its output.
...
Use negative indicies to remove an element.
...
Nowadays, tibbles (an evolution of the data frame that appeared in the tidyverse) replace data frames as they are more practical to handle data sets (you will understand why further below).
...
If you have not installed the package yet, have a look at the section \@ref(the-tidyverse).
...
In Chapter \@ref(working-with-single-tables-in-dplyr) you will see an alternative way of manipulating tibbles with the `dplyr` package.
