...
For example, `plot1` was assigned the following chunk of code:
...
And within that code lays a large data set stored in the object `penguins`.

The clear benefit of assigning data into the above-mentioned objects is that it simplified a lot the code for the figure.
It takes virtually no effort to manipulate the data that the objects contain. 
And it does not take more than 5 seconds to design another figure with a different layout:

```{r palmer-figure2, echo = TRUE}
(plot3 + plot2) / plot1
```
