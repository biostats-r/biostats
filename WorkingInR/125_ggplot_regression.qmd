...
```{r smo-default}
#| fig-alt: >
#|   Scatterplot of penguin bill length against body mass with a smoother
...
```{r smo-lm}
#| fig-alt: >
#|   Scatterplot of penguin bill length against body mass with a linear model
...
```{r smo-glm}
#| fig-alt: >
#|   Plot showing the presence/absence of a diatom species along a pH gradient,
#|   with a fitted logistic model.
...
```{r smo-mod}
mod <- lm(bill_len ~ body_mass + species,
  data = penguins
)
```
...
#|   with the predictions and confidence interval of a linear model added. 
ggplot(penguins, aes(x = body_mass, y = bill_len, fill = species)) +
...
#| fig-alt: >
#|   Plot showing the presence/absence of a diatom species along a pH gradient
#|   with the predictions and confidence interval of a 
