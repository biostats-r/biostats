...
```{r nested-function}
summarise(group_by(penguins, species), mean = mean(bill_len, na.rm = TRUE))
```
...
summarise(penguins_grouped, mean = mean(bill_len, na.rm = TRUE))
```
...
penguins |>
  group_by(species) |>
  summarise(mean = mean(bill_len, na.rm = TRUE))
```
...
# named first argument, penguins pipes into second argument
penguins |>
  lm(bill_len ~ species, data = _) # R 4.2 and newer only
```
...
adelie_summary <- summarise(adelie_grouped, mean_body_mass = mean(body_mass))
```
