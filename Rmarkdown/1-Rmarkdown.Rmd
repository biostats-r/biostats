...
Chapters 1 - 4 of this tutorial will introduce you to R markdown, a tool that lets you combine code and text to  write reproducible documents. 
...
We are going to analyse droughts in Bergen (which raises the fire risk on the coastal heathland (lynghei)). 
Give the document a suitable title and save it in your R studio project.
...
Make your R markdown document show today's date when knitted.
...
```{r formatting2, out.width= "100%", echo=FALSE}
#, out.extra='style="float:right; padding:10px"'
system("pdftoppm -png -x 130 -y 130 -W 220 -H 300 formatting.pdf > formatting.png")
#system("convert formatting.pdf -trim formatting.png")
knitr::include_graphics("formatting.png")
```
...
read_lines("formatting.Rmd", skip_empty_rows = TRUE) %>% 
  paste(., "  ") |> 
  str_replace_all(pattern = "([#\\*\\^\\_~])", "\\\\\\1") |> 
  cat(sep = "\n")
```
...
Write a section titled "Introduction" followed by two short paragraphs of text about _Calluna vulgaris_ and drought (or anything else) to try the different types of formatting.
...
Download file [https://raw.githubusercontent.com/biostats-r/biostats/main/Rmarkdown/data/Florida-precip.csv](https://raw.githubusercontent.com/biostats-r/biostats/main/Rmarkdown/data/Florida-precip.csv) and save it into your RStudio project folder.
The file gives monthly precipitation data from Florida, Bergen since 1983, extracted from [https://seklima.met.no/](https://seklima.met.no/). 
...
Change the column headers so they are easier to work with.
 ...
 <details>
  <summary>Hint</summary>
- Use the `col_names` argument to `read_delim()` 
...
[`gt`](https://gt.rstudio.com/articles/intro-creating-gt-tables.html) is an alternative to `kableExtra`.
...
````
```{r penguin-dens, fig.cap = "Histogram of penguin body mass"}`r ''`
penguins |> 
