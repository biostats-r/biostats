...
To draw a 2D density plot, we will either use `stat_density_2d()` or `geom_density_2d()` with a series of necessary arguments:
```{r 2D, fig.width=10, fig.height=4}
grid.arrange(
...
If you feel like changing the colors used in the color bar guide, you can use `scale_fill/color_gradient()` or `scale_fill/color_viridis_c()` for example:
