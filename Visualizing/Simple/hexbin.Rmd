...
WEB <- c("https://bioceed.uib.no/dropfolder/bioSTATS/Rmd/Basic/Visualizing/Simple/hexbin.html")
GIT <- c("https://github.com/jonathansoule/biostats/blob/master/Basic/Visualizing/Simple/hexbin.Rmd")
library(ggplot2)
```
...
To draw a 2D histogram, we will use `geom_hex()`:
```{r 2D}
ggplot(df, aes(values1, values2)) +
  geom_hex()  
```
