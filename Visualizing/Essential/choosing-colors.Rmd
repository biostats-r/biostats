...
grid.arrange(
  ...
  nrow=1)
```
The colors are âOKâ as long as you do not have more than a handful of groups, but probably this reaches its limits in terms of aesthetics when handling a larger number of groups.
<br/><br/>
...
```
<br/><br/>
And the same plots colored using the matching Hex codes (not that Hex codes start with `#` followed by 6 digits:
```{r manual2, fig.width=10, fig.height=4}
grid.arrange(
  baseplot1 + scale_fill_manual(values=c("#00008B", "#7FFFD4", "#EE9572", "#EE9A00")),
  baseplot2 + scale_fill_gradient(low= "#00008B", high= "#EE9572"),
  nrow=1)
```
<br/><br/>
