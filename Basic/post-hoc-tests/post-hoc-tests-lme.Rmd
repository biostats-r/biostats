...
<br/>
First, we need to fit the linear mixed effect model with `lme()`, which you have certainly already done. Then we run the model in `glht()`. The syntax is `glht(model, linfct=mcp(predictor='Tukey'))` where `predictor` is the predictor variable which levels have to be compared pairwise.
