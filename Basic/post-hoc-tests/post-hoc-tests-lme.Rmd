...
+ `lme()` which fits a linear mixed effect model to your data, and which syntax is `lme(response ~ predictor, random=~1|subject, data = my.dataframe)`
+ `summary()` which computes and displays the result table.

<br/>

The syntax is `glht(model.fit, linfct=mcp(predictor='Tukey'))` where `predictor` is the predictor variable which levels have to be compared pairwise. Let's apply it to our example:
```{r glht}
# fit the model
...
summary(post.hoc)
```
The output shows all six pairwise comparisons, p-values and corresponding stars. 
