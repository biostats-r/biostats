...
<br/>
First, we need to fit the linear model with `lm()`, which you have certainly already done. Then we run the model in `glht()`. The syntax for `glht()` is `glht(model, linfct = mcp(predictor = 'Tukey'))` where `predictor` is the predictor variable which levels have to be compared pairwise. Finally, we run the result in `summary()`.
