...
The following code fits the linear mixed model. The syntax is `lme(response ~ predictor, data = my.dataframe)`, and we must not forget to add the argument `random=~1|subjects` to link the repeated measurements to the corresponding individuals in `rat.ID`:
