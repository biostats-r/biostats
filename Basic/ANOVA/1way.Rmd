...
Indeed, the ANOVA needs to be followed by another test if we want to check which of the groups are different from the others. For that we will need a *post-hoc* test, possibly a [pairwise t-test](http://biostats.w.uib.no/post-hoc-tests-pairwise-comparisons/){target="_blank"} or a [Tukey HSD](http://biostats.w.uib.no/post-hoc-tests-tukey-hsd/){target="_blank"}.
