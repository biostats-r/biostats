...
Indeed, this test tells you about main effects and interactions, but does not tell you which groups are significantly different from others. For that, we will need *post hoc* tests.  We can run [multiple comparisons in linear models with glht() [factorial design]](https://biostats.w.uib.no/post-hoc-tests-multiple-comparisons-in-linear-models-2/){target="_blank"}.
